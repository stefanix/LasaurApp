...
def flash_upload_beaglebone(serial_port, resources_dir):
    ...
    return subprocess.call('%(dude)s -c %(programmer)s -b %(bps)s %(serial_option)s -p %(device)s -C %(dudeconf)s -Uflash:w:%(product)s:i' 
